...
const DEFAULT_EFFECT_OPTIONS = {
  ...
  colors: {
    ...
    leftCars: [
      0xff2d78, // hot pink
      0xff6b00, // electric orange
      0xffe033, // vivid amber
      0xd856bf, // magenta purple
      0xff0055, // neon red
      0xc247ac, // deep pink
      0xff8c42  // warm coral
    ],
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn("WebGL is not supported in this browser. Hyperspeed background animation skipped.");
      return;
    }
    ...
    class App {
      constructor(container, options = {}) {
        ...
        // Safety check: ensure WebGL renderer context initialized correctly
        if (!this.renderer.getContext() || !this.renderer.capabilities) {
    ...
    try {
      ...
      myApp.loadAssets().then(myApp.init).catch(err => {
        console.warn("Failed to initialize Hyperspeed WebGL scene:", err);
      });
    } catch (err) {
      console.warn("Failed to create WebGL app instance, skipping Hyperspeed bg:", err);
    }
