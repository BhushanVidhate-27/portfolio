...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = '16px monospace' }) {
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  update(scroll, direction) {
    // Vertical placement along y coordinate
    const y = this.y - scroll.current - this.extra;
    this.plane.position.y = y;

    const H = this.viewport.height / 2;

    // C-Shape curve mapping (Vertical Arch)
    if (this.bend === 0) {
      ...
      const arc = R - Math.sqrt(Math.max(0, R * R - effectiveY * effectiveY));

      // Shift base position to the left, curving rightwards at top/bottom (flipped curve)
      const base_x = -1.0;
      this.plane.position.x = base_x + arc;

      // Rotate cards facing the center - dampened slightly by 35% for cleaner alignment
      this.plane.rotation.z = Math.sign(y) * Math.asin(effectiveY / R) * 0.65;
    }
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Scale planes for portrait cards
...
export class CircularGalleryApp {
  ...
  createMedias(items, bend, textColor, borderRadius, font) {
    this.mediasImages = items.concat(items); // double array for seamless wrapping
  ...
  onTouchDown(e) {
    if (this.isScrollControlled) return;
    this.isDown = true;
    this.scroll.position = this.scroll.current;
    this.start = e.touches ? e.touches[0].clientY : e.clientY;
  }
  onTouchMove(e) {
    if (this.isScrollControlled || !this.isDown) return;
    const y = e.touches ? e.touches[0].clientY : e.clientY;
    const distance = (this.start - y) * (this.scrollSpeed * 0.02);
